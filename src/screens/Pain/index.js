...
class Pain extends Component {
  ...
  renderAddBloodSugarScreen = () => {
    return (
      <View style={styles.container}>
        ...
        {this.renderNoPainSlider()}
        {this.renderDuration()}
        {this.renderLocationOfPain()}
        {this.renderWhatTriggers()}
        {this.renderWhatRelieves()}
        {this.renderNotes()}
      </View>
    )
  }
  ...
  renderCenterHeader = () => {
    return (
      <Text style={styles.headerText}>Pain</Text>
    )
  }
    ...
    return (
      <View style={{ marginVertical: 10, paddingHorizontal: 20, }}>
        ...
        <Slider
          ...
          onValueChange={(val) => this.setState({ noPainSlider: val })}
        />
      </View>
    )
  }
    ...
    return (
      <View style={{ marginTop: 12, paddingHorizontal: 20, }}>
        ...
        <View style={{ flexDirection: 'row' }}>
          ...
          <Text style={{ marginLeft: 10, fontSize: 16, color: 'lightgray' }}>mins</Text>
        </View>
      </View>
    )
  }

  renderLocationOfPain = () => {
    return (
      <View style={{ marginTop: 12, paddingHorizontal: 20, }}>
        <Text style={{
          color: 'gray',
          fontSize: 18,
        }}>Location of Pain</Text>
        <View style={{ flexDirection: 'row' }}>
    ...
    return (
      <View style={{ marginTop: 12, paddingHorizontal: 20, }}>
        <Text style={{
          color: 'gray',
          fontSize: 18,
        }}>What triggers the pain</Text>
        <View style={{ flexDirection: 'row' }}>
          <TouchableOpacity style={{ flexDirection: 'row', alignItems: 'center', flex: 1, }}>
            <TextInput
              placeholder=''
              value={this.state.whatTriggers}
              onChangeText={(text) => this.setState({ whatTriggers: text })}
              style={[styles.textInput, { paddingRight: 22, flex: 1 }]}
            />
          </TouchableOpacity>
          {/* <Text style={{ marginLeft: 10, fontSize: 16, color: 'rgb(255,49,70)' }}>min</Text> */}
      ...
      <View style={{ marginTop: 12, paddingHorizontal: 20, }}>
        ...
        <View style={{ flexDirection: 'row' }}>
          <TouchableOpacity style={{ flexDirection: 'row', alignItems: 'center', flex: 1, }}>
            <TextInput
              placeholder=''
              value={this.state.whatRelieves}
              onChangeText={(text) => this.setState({ whatRelieves: text })}
              style={[styles.textInput, { paddingRight: 22, flex: 1 }]}
            />
          </TouchableOpacity>
          {/* <Text style={{ marginLeft: 10, fontSize: 16, color: 'rgb(255,49,70)' }}>min</Text> */}
