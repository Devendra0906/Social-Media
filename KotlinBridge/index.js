...
export default class KotlinBridge extends Component {
  render() {
    return (
      <View style={styles.container}>
        <GridView
          itemDimension={130}
          data={Globals.PIXABAY.videos}
          style={styles.gridView}
          renderItem={item => (
            <TouchableWithoutFeedback
              onPress={() => this.onHandleItemPress(item)}>
  ...
  onHandleItemPress(item) {
    ...
    KotlinVideoStream.playVideoStream(item.title, item.url);
  }
